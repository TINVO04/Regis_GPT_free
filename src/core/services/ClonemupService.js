export class ClonemupService {
  constructor(apiKey, options = {}) {
    this.apiKey = `${apiKey || ''}`.trim();
    this.baseUrl = options.baseUrl || 'https://clonemup.com/api';
    this.timeoutMs = Math.max(5000, Number.parseInt(options.timeoutMs ?? 30000, 10) || 30000);
  }

  assertApiKey() {
    if (!this.apiKey) throw new Error('Thiếu Clonemup API key.');
  }
  ...
  async fetchWithTimeout(url, options = {}) {
    ...
    try {
      ...
      if (error.name === 'AbortError') throw new Error('Timeout khi gọi Clonemup API.');
      throw error;
    } finally {
      clearTimeout(timer);
    }
  ...
  async getProfile() {
    this.assertApiKey();
    const data = await this.fetchWithTimeout(this.buildUrl('profile.php', { api_key: this.apiKey }), {
      method: 'GET',
  ...
  async getProduct(productId = 16133) {
    this.assertApiKey();
    const data = await this.fetchWithTimeout(this.buildUrl('product.php', { api_key: this.apiKey, product: productId }), {
      method: 'GET',
      headers: { Accept: 'application/json' },
    });
    return {
      raw: data,
      summary: this.extractProductSummary(data),
    };
  }
  ...
  buildBuyParams({ productId = 16133, amount = 1, coupon = '' } = {}) {
  ...
  async buyProductWithTransport(params, transport = 'get') {
    if (transport === 'get') {
      const data = await this.fetchWithTimeout(this.buildUrl('buy_product', params), {
        method: 'GET',
        headers: { Accept: 'application/json' },
      });
    ...
    if (transport === 'post-form-data') {
      ...
      const data = await this.fetchWithTimeout(this.buildUrl('buy_product'), {
        method: 'POST',
        headers: { Accept: 'application/json' },
        body,
      });
    ...
    throw new Error(`Unsupported Clonemup transport: ${transport}`);
  }

  async buyProduct({ productId = 16133, amount = 1, coupon = '' } = {}) {
    this.assertApiKey();
    ...
    for (const transport of transports) {
      try {
        const { data } = await this.buyProductWithTransport(params, transport);
        if (`${data?.status || ''}`.toLowerCase() !== 'success') {
          const detail = this.summarizeRaw(data);
          const message = detail || data?.msg || data?.message || data?.error || 'Mua Hotmail thất bại.';
          failures.push({ transport, message });
          continue;
        }

        const lines = Array.isArray(data?.data) ? data.data.map((item) => `${item || ''}`.trim()).filter(Boolean) : [];
        return {
          status: data.status,
          message: data.msg || data.message || 'Tạo đơn hàng thành công.',
          transId: data.trans_id || data.transId || '',
          lines,
          raw: data,
    ...
    const summary = failures.map((item) => `${item.transport} => ${item.message}`).join(' || ');
    const aggregateError = new Error(summary || 'Mua Hotmail thất bại.');
    aggregateError.transportFailures = failures;
    throw aggregateError;
  }

  async buyProductInBatches({ totalAmount, productId = 16133, batchSize = 100, coupon = '', onProgress = null } = {}) {
    ...
    for (let offset = 0, batchIndex = 1; offset < requested; offset += safeBatchSize, batchIndex += 1) {
      ...
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        try {
          ...
          const maintenanceStop = failures.length > 0
            ? failures.every((item) => this.isMaintenanceMessage(item?.message || ''))
            : this.isMaintenanceMessage(message);
      ...
      if (lastError) {
        failedBatches.push({
          ...
          productSummary,
          transportFailures: Array.isArray(lastError.transportFailures) ? lastError.transportFailures : [],
        });
        break;
      }
    }
